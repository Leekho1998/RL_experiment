...
class SACAgent:
    def __init__(self, state_dim, action_dim, max_size=1000, batch_size=128, load_file='./sim/saved_model/sac_model/'):
        ...
        self.actor_local = Actor(state_dim, action_dim, hidden_size).to(self.device)
        self.actor_optimizer = optim.Adam(self.actor_local.parameters(), lr=self.learning_rate)     
        
        # Critic Network (w/ Target Network)

        self.critic1 = Critic(state_dim, action_dim, hidden_size, 2).to(self.device)
        self.critic2 = Critic(state_dim, action_dim, hidden_size, 1).to(self.device)
        
        assert self.critic1.parameters() != self.critic2.parameters()
        
        self.critic1_target = Critic(state_dim, action_dim, hidden_size).to(self.device)
        self.critic1_target.load_state_dict(self.critic1.state_dict())

        self.critic2_target = Critic(state_dim, action_dim, hidden_size).to(self.device)
